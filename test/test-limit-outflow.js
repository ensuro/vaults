...
const variants = [
  {
    ...
    fixture: async () => {
      ...
      const { MultiStrategyERC4626, ERC1967Proxy, LimitOutflowModifier, strategies, adminAddr, currency, admin } = ret;
  ...
  {
    name: "AccessManagedProxy+LOM+AccessManagedMSV",
    tagit: tagit,
    fixture: async () => {
      const ret = await setUp();

      const { AccessManagedProxy, AccessManagedMSV, LimitOutflowModifier, strategies, currency, admin } = ret;
      ...
      async function deployVault(strategies_, initStrategyDatas, depositQueue, withdrawQueue) {
        ...
        const deploymentTransaction = proxy.deploymentTransaction();
        const vault = await ethers.getContractAt("AccessManagedMSV", await ethers.resolveAddress(proxy));
        vault.deploymentTransaction = () => deploymentTransaction;
        ...
        // Use getContractAt instead of attach to avoid errors with emit assertions
        const vaultAsLOM = await ethers.getContractAt("LimitOutflowModifier", await ethers.resolveAddress(proxy));
        await vaultAsLOM.connect(admin).LOM__setLimit(DAY, _A(1000));

        await makeAllViewsPublic(acMgr.connect(admin), vaultAsLOM);

        // Setup LP role
        await acMgr.connect(admin).labelRole(LP_ROLE, "LP_ROLE");
        for (const method of ["withdraw", "deposit", "mint", "redeem", "transfer"]) {
          await acMgr
            .connect(admin)
            .setTargetFunctionRole(vault, [vault.interface.getFunction(method).selector], LP_ROLE);
        }
      ...
      async function grantRole(_, role, user) {
        if (role === "LP_ROLE") role = LP_ROLE;
        if (role === "LOM_ADMIN") role = LOM_ADMIN;
        await acMgr.connect(admin).grantRole(role, user, 0);
      }
  ...
  },
  {
    ...
    fixture: async () => {
      ...
      async function deployVault(strategies_, initStrategyDatas, depositQueue, withdrawQueue) {
        ...
        return {
          vault,
          lom: vault, // Cuando quede solamente esta variant entonces debería de retornar solamente vault y utilizarlo en lugar de lom en los tests
        };
      }
...
variants.forEach((variant) => {
  describe(`${variant.name} contract tests`, function () {
    it("Initializes the vault correctly", async () => {
      ...
      const { vault, lom } = await deployVault(1); // Deberiamos de dejar de destructurar LOM y solo usar vault?
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV") expect(await vault.getOutflowLimit()).to.equal(_A(1000));
      else expect(await lom.LOM__getLimit()).to.equal(_A(1000));
    });
    ...
    it("Checks that change in slot size resets the limits", async () => {
      ...
      await expect(vault.connect(lp).withdraw(_A(600), lp, lp)).not.to.be.reverted;
      // Fails because it reached the limit in the last two days
      if (variant.name === "AMProxy+OutflowLimitedAMMSV") {
        await expect(lom.connect(admin).setupOutflowLimit(DAY / 2, _A(500)))
          .to.emit(lom, "LimitChanged")
          .withArgs(DAY / 2, _A(500));
        expect(await lom.getOutflowLimit()).to.equal(_A(500));
        expect(await lom.getOutflowLimitSlotSize()).to.equal(DAY / 2);
      } else {
        await expect(lom.connect(admin).LOM__setLimit(DAY / 2, _A(500)))
          .to.emit(lom, "LimitChanged")
          .withArgs(DAY / 2, _A(500));
    ...
    it("Allows accumulated withdrawals up to the daily limit and prevents exceeding it", async () => {
      ...
      await expect(vault.connect(lp).deposit(_A(5000), lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(5000));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(5000));
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(0);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(0);

      await expect(vault.connect(lp).withdraw(_A(300), lp, lp)).not.to.be.reverted;
      await expect(vault.connect(lp).withdraw(_A(400), lp, lp)).not.to.be.reverted;
      await expect(vault.connect(lp).withdraw(_A(299), lp, lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(-999));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(-999));

      await expect(vault.connect(lp).withdraw(_A(1), lp, lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(-1000));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(-1000));
      await expect(vault.connect(lp).withdraw(_A(1), lp, lp)).to.be.revertedWithCustomError(lom, "LimitReached");
    ...
    it("Allows accumulated withdrawals up to the daily limit and prevents exceeding it - mint/redeem", async () => {
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(-450), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(-450), 1n);
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(-999), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(-999), 1n);
      ...
      now = await helpers.time.latest();
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(0), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(0), 1n);
      await helpers.time.increase(DAY);
      now = await helpers.time.latest();

      await expect(vault.connect(lp).redeem(_A(500), lp, lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(-750));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(-750));
      await expect(vault.connect(lp).redeem(_A(165), lp, lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(-997.5), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(-997.5), 1n);
    ...
    it("Allows accumulated withdrawals up to the daily limit and prevents exceeding it - mint/redeem/deposit/withdraw", async () => {
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(3000));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(3000));
      await helpers.time.increase(DAY * 15);
      ...
      await expect(vault.connect(lp).redeem(_A(300), lp, lp)).not.to.be.reverted;
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(-300));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(-300));
      await expect(vault.connect(lp).withdraw(_A(400), lp, lp)).not.to.be.reverted;
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.equal(_A(2000));
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.equal(_A(2000));
      await currency.connect(lp).transfer(await strategies[0].other(), _A(2500)); // Give 2500 for free to the vault, now 1 share = 1.5 assets
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(-1000), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(-1000), 1n);
      await expect(vault.connect(lp).withdraw(_A(1), lp, lp)).to.be.revertedWithCustomError(lom, "LimitReached");
      await expect(vault.connect(lp).redeem(_A(1), lp, lp)).to.be.revertedWithCustomError(lom, "LimitReached");
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV")
        expect(await lom.getAssetsDelta(await lom.makeOutflowSlot(slotSize, now))).to.closeTo(_A(-750), 1n);
      else expect(await lom.LOM__getAssetsDelta(await lom.LOM__makeSlot(slotSize, now))).to.closeTo(_A(-750), 1n);
    ...
    it("Sets and resets delta using LOM__changeDelta correctly", async () => {
      ...
      if (variant.name === "AMProxy+OutflowLimitedAMMSV") {
        const slotIndex = await lom.makeOutflowSlot(slotSize, currentTimestamp);

        await expect(lom.connect(admin).changeDelta(slotIndex, _A(1500)))
          .to.emit(lom, "DeltaManuallySet")
          .withArgs(slotIndex, 0, _A(1500));
        await expect(lom.connect(admin).changeDelta(slotIndex, _A(-250)))
          .to.emit(lom, "DeltaManuallySet")
